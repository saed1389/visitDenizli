...
@section('title') Visit Denizli - Yatırım Fırsat Güncelle @endsection
...
@section('content')
    ...
    <div class="px-md-4 px-2 py-2 page-header" data-bs-theme="none">
        <div class="d-flex align-items-center">
            ...
            <ol class="breadcrumb mb-0 bg-transparent">
                ...
                <li class="breadcrumb-item"><a href="{{ route('admin.investment.index') }}" >Yatırım Fırsatları Listesi</a></li>
                <li class="breadcrumb-item active" aria-current="page" title="App">Yatırım Fırsat Güncelle</li>
            </ol>
        </div>
    ...
    <div class="ps-md-4 pe-md-3 px-2 py-3 page-body">
        <h3 class="title-font mb-3">Yatırım Fırsat Güncelle</h3>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body card-main-one">
                        <form action="{{ route('admin.investment.update', $investment->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-sm-6">
                                    <label for="name" class="form-label"><strong>Yatırım Fırsat Adı (TR) <span class="text-danger">*</span></strong></label>
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Yatırım Fırsat Adı (TR)" value="{{ $investment->name }}" required>
                                    @error('name')
                                ...
                                <div class="col-sm-6">
                                    <label for="name_en" class="form-label"><strong>Yatırım Fırsat Adı (EN)</strong></label>
                                    <input type="text" class="form-control" id="name_en" name="name_en" placeholder="Yatırım Fırsat Adı (EN)" value="{{ $investment->name_en }}" >
                                    @error('name_en')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-12 mt-3">
                                    ...
                                    <textarea name="description" class="form-control" id="ckeditor_tr" required >{{ $investment->description }}</textarea>
                                    @error('description')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-12 mt-3">
                                    ...
                                    <textarea name="description_en" class="form-control" id="ckeditor_en" >{{ $investment->description_en }}</textarea>
                                ...
                                <div class="col-sm-3 mt-3">
                                    <img src="{{ $investment->image ? asset($investment->image) : asset('panel/assets/images/def.png') }}" id="showImage" class="img-thumbnail" alt="" >
                                </div>
                                <div class="col-sm-3 mt-3">
                                    <label for="status" class="form-label"><strong>Durum</strong></label>
                                    <div class="my-3">
                                        <input id="active" name="status" type="radio" value="1" class="form-check-input" @checked($investment->status == 1 )>
                                        <label class="form-check-label" for="active">Acik</label>
                                        <input id="deactivate" name="status" type="radio" value="0" class="form-check-input" @checked($investment->status == 0 )>
                ...
                upload() {
                    return this.loader.file
                        .then(file => new Promise((resolve, reject) => {
                            ...
                            fetch("{{ route('admin.investment.upload') }}", {
                                method: 'POST',
                                headers: {
