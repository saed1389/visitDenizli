...
@section('title') Visit Denizli - Kullanıcı Güncelle @endsection
...
@section('content')
    <div class="px-md-4 px-2 py-2 page-header" data-bs-theme="none">
        <div class="d-flex align-items-center">
            ...
            <ol class="breadcrumb mb-0 bg-transparent">
                ...
                <li class="breadcrumb-item"><a href="{{ route('admin.user.index') }}" >Kullanıcı Listesi</a></li>
                <li class="breadcrumb-item active" aria-current="page" title="App">Kullanıcı Güncelle</li>
            </ol>
        </div>
    ...
    <div class="ps-md-4 pe-md-3 px-2 py-3 page-body">
        <h3 class="title-font mb-3">Kullanıcı Güncelle</h3>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body card-main-one">
                        <form action="{{ route('admin.user.update', $user->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <label for="name" class="form-label"><strong>Kullanıcı Adı <span class="text-danger">*</span></strong></label>
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Kullanıcı Adı " value="{{ $user->name }}" required>
                                    @error('name')
                                ...
                                <div class="col-sm-6">
                                    <label for="email" class="form-label"><strong>Kullanıcı E-postası <span class="text-danger">*</span></strong></label>
                                    <input type="email" class="form-control" id="email" name="email" placeholder="Kullanıcı E-postası" value="{{ $user->email }}" required >
                                    @error('email')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-6 mt-3">
                                    ...
                                    <input type="password" class="form-control" id="password" name="password" placeholder="Kullanıcı Şifresi" value="" >
                                    @error('password')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-3 mt-3">
                                    ...
                                    <select class="form-select" name="county_id" id="county_id" required >
                                        ...
                                        @foreach($counties as $county)
                                            <option value="{{ $county->id }}" @selected($user->county_id == $county->id)>{{ $county->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('county_id')
                                    <span class="text-danger">
                                ...
                                <div class="col-sm-3 mt-3">
                                    ...
                                    <div class="my-3">
                                        <input id="active" name="status" type="radio" value="1" class="form-check-input" @checked($user->status == 1)>
                                        <label class="form-check-label" for="active">Acik</label>
                                        <input id="deactivate" name="status" type="radio" value="0" class="form-check-input" @checked($user->status == 0)>
                                ...
                                <div class="col-12 text-end">
                                    <a href="{{ route('admin.user.index') }}" class="btn btn-outline-secondary">İptal</a>
