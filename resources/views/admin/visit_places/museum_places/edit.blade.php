...
@section('title') Visit Denizli - Müze ve Sanat Galeri Güncelle @endsection
...
@section('content')
    ...
    <div class="px-md-4 px-2 py-2 page-header" data-bs-theme="none">
        <div class="d-flex align-items-center">
            ...
            <ol class="breadcrumb mb-0 bg-transparent">
                ...
                <li class="breadcrumb-item"><a href="{{ route('admin.museum-places.index') }}" >Müzeler ve Sanat Galerileri Listesi</a></li>
                <li class="breadcrumb-item active" aria-current="page" title="App">Müze ve Sanat Galeri Güncelle</li>
            </ol>
        </div>
    ...
    <div class="ps-md-4 pe-md-3 px-2 py-3 page-body">
        <h3 class="title-font mb-3">Müze ve Sanat Galeri Güncelle</h3>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body card-main-one">
                        <form action="{{ route('admin.museum-places.update', $place->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-sm-3">
                                    <label for="name" class="form-label"><strong>Müze ve Sanat Galeri Adı (TR)<span class="text-danger">*</span></strong></label>
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Müze ve Sanat Galeri Adı (TR)" value="{{ $place->name }}" required>
                                    @error('name')
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <input type="text" class="form-control" id="name_en" name="name_en" placeholder="Müze ve Sanat Galeri Adı (EN)" value="{{ $place->name_en }}">
                                    @error('name_en')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <select class="form-select" name="category" id="category" required >
                                        ...
                                        <option value="1" @selected($place->category == 1)>Müze</option>
                                        <option value="2" @selected($place->category == 2)>Sanat Galeri</option>
                                    </select>
                                    @error('category')
                                    <span class="text-danger">
                                        {{ $message }}
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <select class="form-select" name="county_id" id="county_id" required >
                                        ...
                                        @foreach($counties as $county)
                                            <option value="{{ $county->id }}" @selected($place->county_id == $county->id)>{{ $county->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('county_id')
                                    <span class="text-danger">
                                ...
                                <div class="col-sm-12 mt-3">
                                    ...
                                    <textarea name="description" class="form-control" id="ckeditor_tr" >{{ $place->description }}</textarea>
                                    @error('description')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-12 mt-3">
                                    ...
                                    <textarea name="description_en" class="form-control" id="ckeditor_en" >{{ $place->description_en }}</textarea>
                                ...
                                <div class="col-sm-6 mt-3">
                                    ...
                                    <input type="text" class="form-control" id="latitude" name="latitude" placeholder="Müze ve Sanat Galeri Enlem " value="{{ $place->latitude }}" >
                                    @error('latitude')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-6 mt-3">
                                    <label for="longitude" class="form-label"><strong>Müze ve Sanat Galeri Boylam</strong></label>
                                    <input type="text" class="form-control" id="longitude" name="longitude" placeholder="Müze ve Sanat Galeri Boylam " value="{{ $place->longitude }}">
                                    @error('longitude')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-6 mt-3 d-flex flex-wrap" id="preview-container">
                                    ...
                                    @foreach($images as $image)
                                        <div class="image-preview position-relative m-2">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm remove-existing-image position-absolute top-0 end-0" data-image="{{ $image }}" data-id="{{ $place->id }}">X</button>
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                <div class="col-sm-2 mt-3">
                                    <img src="{{ $place->banner_image ? asset($place->banner_image) : asset('panel/assets/images/def.png') }}" id="showBannerImage" class="img-thumbnail" alt="" >
                                </div>

                                <div class="col-sm-3 mt-3">
                                    <label for="status" class="form-label"><strong>Durum</strong></label>
                                    <div class="my-3">
                                        <input id="active" name="status" type="radio" value="1" class="form-check-input" @checked($place->status == 1)>
                                        <label class="form-check-label" for="active">Acik</label>
                                        <input id="deactivate" name="status" type="radio" value="0" class="form-check-input" @checked($place->status == 0)>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $(document).on('click', '.remove-existing-image', function () {
                    ...
                    $.ajax({
                        url: "{{ route('admin.museum-places.delete-image') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            image: imageToRemove,
