            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body card-main-one">
                        <form action="{{ route('admin.history-places.update', $history->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-sm-4">
                                    <label for="name" class="form-label"><strong>Tarihi Mekan Adı (TR)<span class="text-danger">*</span></strong></label>
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Tarihi Mekan Adı (TR)" value="{{ $history->name }}" required>
                                    @error('name')
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" class="form-control" id="name_en" name="name_en" placeholder="Tarihi Mekan Adı (EN)" value="{{ $history->name_en }}">
                                    @error('name_en')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <select class="form-select" name="county_id" id="county_id" required >
                                        ...
                                        @foreach($counties as $county)
                                            <option value="{{ $county->id }}" @selected($history->county_id == $county->id)>{{ $county->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('county_id')
                                    <span class="text-danger">
                                ...
                                <div class="col-sm-12 mt-3">
                                    ...
                                    <textarea name="description" class="form-control" id="ckeditor_tr" >{{ $history->description }}</textarea>
                                    @error('description')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-12 mt-3">
                                    ...
                                    <textarea name="description_en" class="form-control" id="ckeditor_en" >{{ $history->description_en }}</textarea>
                                ...
                                <div class="col-sm-6 mt-3">
                                    ...
                                    <input type="text" class="form-control" id="latitude" name="latitude" placeholder="Tarihi Mekan Enlem " value="{{ $history->latitude }}" >
                                    @error('latitude')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-6 mt-3">
                                    ...
                                    <input type="text" class="form-control" id="longitude" name="longitude" placeholder="Tarihi Mekan Boylam " value="{{ $history->longitude }}">
                                    @error('longitude')
                                    <span class="text-danger">
                                        {{ $message }}
                                    </span>
                                ...
                                <div class="col-sm-6 mt-3 d-flex flex-wrap" id="preview-container">
                                    ...
                                    @foreach($images as $image)
                                        <div class="image-preview position-relative m-2">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm remove-existing-image position-absolute top-0 end-0" data-image="{{ $image }}" data-id="{{ $history->id }}">X</button>
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                <div class="col-sm-2 mt-3">
                                    <img src="{{ $history->banner_image ? asset($history->banner_image) : asset('panel/assets/images/def.png') }}" id="showBannerImage" class="img-thumbnail" alt="" >
                                </div>

                                <div class="col-sm-3 mt-3">
                                    <label for="status" class="form-label"><strong>Durum</strong></label>
                                    <div class="my-3">
                                        <input id="active" name="status" type="radio" value="1" class="form-check-input" @checked($history->status == 1)>
                                        <label class="form-check-label" for="active">Acik</label>
                                        <input id="deactivate" name="status" type="radio" value="0" class="form-check-input" @checked($history->status == 0)>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $(document).on('click', '.remove-existing-image', function () {
                    ...
                    $.ajax({
                        url: "{{ route('admin.history-places.delete-image') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            image: imageToRemove,
