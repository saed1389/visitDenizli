    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($query) {
                $edit = "<a href='".route('admin.news.edit', $query->id)."' class='btn btn-sm btn-primary me-1 '><i class='fa fa-edit'></i></a>";
                $delete = "<a href='".route('admin.news.destroy', $query->id)."' class='btn btn-sm btn-danger delete-item' id='delete'><i class='fa fa-trash'></i></a>";

                return $edit.$delete;
            })->addColumn('image', function ($query) {
                return '<img src="'.asset($query->image).'" width="50" height="50" />';
            })->addColumn('status', function (News $news) {
            ...
            ->addColumn('created_at', function (News $news) {
                return $news->created_at->diffForHumans();
            })
            ->addColumn('county_id', function (News $news) {
                return $news->county->name;
            })
            ->addColumn('created_by', function (News $news) {
                return $news->createdBy->name;
            })
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('image')
                ...
                ->orderable(false)
                ->searchable(false),
            Column::make('name')->title('Haber Başlığı'),
