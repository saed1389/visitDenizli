    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($query) {
                $edit = "<a href='".route('admin.events.edit', $query->id)."' class='btn btn-sm btn-primary me-1 '><i class='fa fa-edit'></i></a>";
                $delete = "<a href='".route('admin.events.destroy', $query->id)."' class='btn btn-sm btn-danger delete-item' id='delete'><i class='fa fa-trash'></i></a>";
                ...
                <div class='form-check form-switch' style='justify-self: center;'>
                    <input class='form-check-input switch-input active' type='checkbox' data-id='{$event->id}' role='switch' {$status}>
                </div>";
            })
            ->addColumn('created_at', function (Event $event) {
                return $event->created_at->diffForHumans();
            })
            ->addColumn('county_id', function (Event $event) {
                return $event->county->name;
            })
            ->addColumn('created_by', function (Event $event) {
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('image')
                ...
                ->orderable(false)
                ->searchable(false),
            Column::make('name')->title('Etkinlik adı'),
            Column::make('start_date')->title('Başlangıç Tarihi'),
            Column::make('end_date')->title('Bitiş Tarihi'),
